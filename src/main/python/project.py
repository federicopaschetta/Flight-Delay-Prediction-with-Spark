...
# Gets categorical variables
categorical_vars = [cols for cols in df.columns if any(c.isalpha() for c in str(df.head(1)[0][cols]))]
categorical_vars = [cols for cols in categorical_vars if cols not in fake_categorical]
...
# Creates indexes for Dest col and transforms df creating new col with the right index for each row
dest_indexer = StringIndexer(inputCol='Dest', outputCol='Dest_Index')
df = dest_indexer.fit(df).transform(df)

# Creates indexes for Origin col and transforms df creating new col with the right index for each row
...
# Encodes with OneHotEncoding Origin based on col previously created and adds new OneHotEncoding column
origin_encoder = OneHotEncoder(inputCol='Origin_Index', outputCol='Origin_OneHot')
df = origin_encoder.fit(df).transform(df)

# Encodes with OneHotEncoding Dest based on col previously created and adds new OneHotEncoding column
dest_encoder = OneHotEncoder(inputCol='Dest_Index', outputCol='Dest_OneHot')
df = dest_encoder.fit(df).transform(df)
...
print(deleting_cat_vars)


# Renames column removing 'OneHot' from column name
df = df.drop(*deleting_cat_vars)
df = df.withColumnRenamed('Origin_OneHot', 'Origin')
df = df.withColumnRenamed('Dest_OneHot', 'Dest')
